...
class SystemPrivacyDropbearConfig(SystemPrivacy):
    def __call__(self):
        ...
        if (data is not None) and ("RootPasswordAuth 'on'" in data):
            alert_root_login = alert("Root user can login via ssh. Consider disabling this for security purposes.", ALERT_TYPE.PRIVACY, SEVERITY.INFO)
            alert_root_login.alert()
        if "PasswordAuth 'on'" in data:
            alert_general_login = alert("Password login via ssh is allowed. Consider only allowing keypair login via ssh.", ALERT_TYPE.PRIVACY, SEVERITY.INFO)
