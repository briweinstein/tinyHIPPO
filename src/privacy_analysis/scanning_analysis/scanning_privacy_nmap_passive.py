...
ports_allow_udp = [7, 9, 53, 80, 123, 137, 138, 139, 443, 500, 593, 1433, 1434, 1719, 4500, 5000, 5432, 5060, 9200,
                   10000]
...
class ScanningPrivacyNmapPassive(ScanningPrivacy):
    ...
    def __call__(self, ip_to_mac):
        ...
        for ip in list(results_top["scan"].keys()):
            self._inspect_open_ports(ip, ip_to_mac[ip], results_top["scan"], "tcp", ports_allow_tcp,
                                     ports_severe_alert_tcp)
            self._inspect_open_ports(ip, ip_to_mac[ip], results_top["scan"], "udp", ports_allow_udp,
                                     ports_severe_alert_udp)
        for ip in list(results_specific["scan"].keys()):
            self._inspect_open_ports(ip, ip_to_mac[ip], results_specific["scan"], "tcp", [], [])

    def _inspect_open_ports(self, ip, mac, results, port_type, ports_allow, ports_severe_alert):
        """
